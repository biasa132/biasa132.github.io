# Ultimate Bahasa Lo REPL Final v6 – Full Final Script
...
def proses_baris(b):
    ...
    if b.lower() == "admin":
        ...
        if password == "rahasia123":
            ...
            if choice=="1":
                ...
                print("1. Tambah repo pit\n2. Hapus repo pit\n3. Lihat repo pit")
                sub = input("Pilih opsi: ").strip()
                if sub=="1":
                    tipe = input("Masukkan tipe repo (deb/git/http): ").strip()
                    ...
                    for i,r in enumerate(repos,1):
                        print(f"{i}. {r}")
                    idx = input("Masukkan nomor repo yg ingin dihapus: ").strip()
                    if idx.isdigit() and 1<=int(idx)<=len(repos):
                        repos.pop(int(idx)-1)
                        with open(repo_list_file,"w") as f:
                            f.write("\n".join(repos))
                        print("Repo berhasil dihapus!")
                elif sub=="3":
                    with open(repo_list_file,"r") as f:
                        repos = [line.strip() for line in f if line.strip()]
                    print("Daftar repo pit:")
                    for r in repos:
                        status = "OK" if test_mirror(r.split()[1]) else "Bad"
                        print(f"{r} : {status}")
            return None
        else:
    ...
    if b.startswith("pit "):
        parts = b.split()
        if len(parts)<2:
            print("Format: pit <command> [package]")
            return None
        cmd = parts[1]
        if cmd=="install" and len(parts)>=3:
            pkg_name = parts[2]
            print(f"Pit install {pkg_name}")
            pit_download_package(pkg_name)
            return None
        elif cmd=="search":
            print("Fitur search masih belum implementasi full")
            return None
        elif cmd=="show":
            print("Fitur show masih belum implementasi full")
            return None
        elif cmd=="list":
            print("Daftar paket terinstall:")
            for p in os.listdir(packages_folder):
                print(f"- {p}")
            return None
        else:
            print(f"Unknown pit command: {cmd}")
            return None
    ...
    if b.startswith("jalankan "):
        ...
        if os.path.exists(file_blo):
            try:
                exec(open(file_blo).read(), globals(), variabel)
            except Exception as e:
