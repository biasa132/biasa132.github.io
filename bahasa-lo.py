# Ultimate Bahasa Lo REPL + Pit Final v6
...
def pit_install(pkg):
    ...
    for repo in repos:
        ...
        if not mirror_ok:
            continue
        # Cek jika ada package .deb
        url_pkg = f"{repo}{pkg}"
        try:
            response = urllib.request.urlopen(url_pkg)
            # bikin folder packages
            out_dir = os.path.join(packages_folder, pkg)
            os.makedirs(out_dir, exist_ok=True)
            out_file = os.path.join(out_dir, f"{pkg}.deb")
            urllib.request.urlretrieve(url_pkg, out_file)
            print(f"Downloaded {pkg} -> {out_file}")
            found = True
            break
        except:
            continue
