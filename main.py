...
def eksekusi_perintah(baris, konteks):
    baris = baris.strip()
    if not baris:
        return
    ...
    try:
        ...
        cetak(f"Eksekusi sebagai Python: {kode_python}")
        exec(kode_python, konteks)
        return
    except Exception as e:
        debug_log(f"Python gagal: {e}")

    # ==========================
    # 2. COBA COMMAND LINUX
    # ==========================
    try:
        cetak(f"Coba sebagai Linux command: {baris}")
        subprocess.run(
            baris,
            shell=True,
            check=True
        ...
        debug_log(f"Linux command gagal: {e}")

    # ==========================
    # 3. ERROR FINAL
    # ==========================
    cetak_error("Perintah tidak dikenali")
